...
If one of these rules are broken, you should *stop* refactoring and *revert* your changes.
====
...
git commit -m "Replace Nested Conditinal with Guard Clause"
git push
----
****
...
Apply the _Extract Method_ refactoring operation to replace the two lines that actually adds an element and create the private method `void addElement(Object obj)`.
